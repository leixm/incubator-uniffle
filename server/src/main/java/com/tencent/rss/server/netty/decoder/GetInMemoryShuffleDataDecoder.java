...
public class GetInMemoryShuffleDataDecoder extends ByteToMessageDecoder {
  ...
  private GetInMemoryShuffleDataMessage getInMemoryShuffleDataMessage = new GetInMemoryShuffleDataMessage();

  public GetInMemoryShuffleDataDecoder() {
    super();
  }
  ...
  protected void decode(ChannelHandlerContext ctx, ByteBuf in, List<Object> out) {
    ...
    switch (state) {
      case READ_MAGIC_BYTE:
        ...
        switch (magicByte) {
          case MessageConstants.GET_IN_MEMORY_SHUFFLE_DATA_MAGIC_BYTE:
      ...
      case READ_TASK_FETCH_DATA_INFO:
        ...
        if (in.readableBytes() < requiredBytes + 3 * Integer.BYTES + Long.BYTES) {
          return;
        }
        getInMemoryShuffleDataMessage.readMessageInfo(in, requiredBytes);
        out.add(getInMemoryShuffleDataMessage);
        return;
      default:

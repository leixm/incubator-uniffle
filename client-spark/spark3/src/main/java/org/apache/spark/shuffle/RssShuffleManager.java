...
public class RssShuffleManager implements ShuffleManager {
  ...
  public <K, C> ShuffleReader<K, C> getReaderImpl(
    ...
    Set<ShuffleServerInfo> needRequestServers = allPartitionToServers.entrySet().stream()
        .filter(entry -> entry.getKey() >= startPartition && entry.getKey() <= endPartition)
        .flatMap(entry -> entry.getValue().stream())
        .collect(Collectors.toSet());
    long start = System.currentTimeMillis();
    Roaring64NavigableMap blockIdBitmap = shuffleWriteClient.getShuffleResult(
        clientType, needRequestServers, rssShuffleHandle.getAppId(),
        shuffleId, startPartition, endPartition);
    ...
    return new RssShuffleReader<K, C>(
        ...
        (int) readBufferSize,
        partitionNum,
        RssUtils.shuffleBitmapToPartitionBitmap(blockIdBitmap),
        taskIdBitmap,
        readMetrics);
  }
